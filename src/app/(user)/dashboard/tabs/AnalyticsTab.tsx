  ...
  const quotaGauge = (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-card rounded-2xl p-6">
      ...
      <div className="flex items-baseline gap-2 mb-3">
        ...
        <span className="text-sm text-muted-foreground">/ {data.user.requestLimit === -1 ? "Unlimited" : data.user.requestLimit.toLocaleString()} used</span>
      </div>
      ...
      <p className="text-xs text-muted-foreground mt-2">
        {data.user.requestLimit === -1 ? "Uncapped monthly capacity" : `${requestsRemaining.toLocaleString()} requests remaining this month`}
      </p>
    </motion.div>
  );
