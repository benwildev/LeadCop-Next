...
import { CreditCard, ArrowUpRight, Download, FileText, Loader2 } from "lucide-react";
...
export default function BillingTab() {
  const { data: billingData, isLoading, error } = useQuery<{ requests: BillingRequest[] }>({
    queryKey: ["/api/user/billing"],
    queryFn: async () => {
      const response = await axiosSecure.get("/api/user/billing");
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="glass-card rounded-2xl p-6">
        ...
        {isLoading ? (
          ...
          <p className="text-center text-sm text-red-400 py-6">Failed to load billing history</p>
