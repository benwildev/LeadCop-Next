...
export function CloudinaryUpload({ value, onChange, label = "Upload Image", hint, accept = "image/*" }: Props) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      fd.append("file", file);
      const res = await axiosSecure.post("/api/admin/upload", fd, {
    ...
    <div className="space-y-1.5">
      ...
      {uploadMutation.error && (
        <p className="flex items-center gap-1 text-xs text-destructive">
          <X className="h-3 w-3" /> {uploadMutation.error instanceof Error ? uploadMutation.error.message : "Upload failed"}
        </p>
      )}
